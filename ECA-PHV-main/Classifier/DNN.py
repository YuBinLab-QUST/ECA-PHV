...
data=data[:,1:]
[m1,n1]=np.shape(data)
label1=np.ones((int(10858),1))
label2=np.zeros((int(10858),1))
...
def get_DNN_model(input_dim,out_dim):
     model = Sequential()
     model.add(Dense(int(input_dim), activation='relu', kernel_initializer='glorot_normal', name='High_dim_feature_1'))
     model.add(Dropout(0.5))
     model.add(Dense(int(input_dim/2), activation='relu', kernel_initializer='glorot_normal', name='High_dim_feature_2'))
     model.add(Dropout(0.5))
     model.add(Flatten())
     model.add(Dense(int(input_dim/4), activation='relu', kernel_initializer='glorot_normal', name='High_dim_feature'))
     model.add(Dropout(0.5))
